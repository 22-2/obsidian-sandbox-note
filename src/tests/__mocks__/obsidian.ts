...
export class Workspace {
	...
	getLeavesOfType = vi.fn((type: string): any[] => {
...
export class ItemView extends Component {
	...
	async setState(state: any, result: any): Promise<void> {
		this.state = state;
		// The original setState returns a Promise that resolves when the view is updated
		return Promise.resolve();
	}
...
export class Keymap {
	...
	bindKey = vi.fn();
	unbindKey = vi.fn();
	setDefaultBindings = vi.fn();
	getBinding = vi.fn();
	isModifierEvent = vi.fn(() => false);
}
